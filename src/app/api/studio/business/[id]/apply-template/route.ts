//src/app/api/studio/business/[id]/apply-template/route.ts
...
function clampTypography(v: any, fallback = "system") {
    // Guardamos key (ej: "inter", "system", etc.) o string acotado
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            // ✅ 3) BORRAR TODAS las páginas del negocio antes de recrear
            // Esto asegura que páginas viejas (que el template nuevo no trae) no queden “colgadas”.
            await tx.businessPage.deleteMany({
                where: { businessId },
            });

            // ✅ 4) Crear páginas desde template (limpiando imágenes)
            for (const p of tPages) {
                const slug = safeStr(p?.slug);
                const title = safeStr(p?.title);
                if (!slug || !title) continue;

                const contentRaw = safeJson<any>(p?.content, []);
                const content = stripImagesFromContent(contentRaw);

                await tx.businessPage.create({
                    data: {
                        businessId,
                        slug,
                        title,
        ...
        console.error("POST /api/studio/business/[id]/apply-template error:", e);
