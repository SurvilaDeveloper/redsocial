//src/app/api/friendship/route.ts

import { NextRequest, NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";
import auth from "@/auth";
import { getRelationshipState, setFriendship } from "@/lib/relationship";
 ...
 */
    ...
    */
    ...
    const relState = await getRelationshipState(friendOne, userId);
    ...
    try {
        if (subject === "get") {
            ...
            if (relState === 3) {
                setFriendshipSuccess = await setFriendship(friendOne, userId, { req1: 1, res1: 1, req2: 1, res2: 1 })
                newRelState = 8
            } else {
                setFriendshipSuccess = await setFriendship(friendOne, userId, { req1: 1, res1: 0, req2: 0, res2: 0 })
                newRelState = 2
            }
            ...
            setFriendshipSuccess = await setFriendship(friendOne, userId, { req1: 0, res1: 1, req2: 1, res2: 0 })
            newRelState = 4
        } else if (subject === "deleteFriendship") {
            setFriendshipSuccess = await setFriendship(friendOne, userId, { req1: 0, res1: 1, req2: 0, res2: 0 })
            newRelState = 6
        } else if (subject === "cancelRequest") {
            setFriendshipSuccess = await setFriendship(friendOne, userId, { req1: 0, res1: 0, req2: 0, res2: 0 })
            newRelState = 1
        } else {
