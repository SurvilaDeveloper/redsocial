// src/components/custom/ImagesSwiperSites.tsx
"use client";
...
export const ImagesSwiperSites: React.FC<ImagesSwiperSitesProps> = ({
    ...
    // ✅ imageArray estable (evita recreaciones innecesarias)
    const imageArray: SwiperImageSites[] = useMemo(() => {
        return Object.values(mediaMap ?? {}).map((item, index) => ({
            id: item.id,
            imageUrl: item.url,
    ...
    const setSlide = (index: number) => {
        ...
        // si no hay imágenes, dejamos el estado en 0 y no disparamos callbacks
    ...
    // ✅ si cambia el largo, clamp para evitar índices inválidos (incluye caso length=0)
    useEffect(() => {
        if (imageArray.length === 0) {
            if (!isControlled) setInternalSlide(0);
            return;
    ...
    const nextSlide = () => {
        if (!hasImages) return;
        const next = currentSlide >= imageArray.length - 1 ? imageArray.length - 1 : currentSlide + 1;
        setSlide(next);
        setDragOffset(0);
    };
    ...
    const finalizeDrag = () => {
        ...
        const slideElements = Array.from(container.querySelectorAll<HTMLElement>(".carousel-slide-item"));
    ...
    const trackStyle: React.CSSProperties = useMemo(() => {
        ...
        return {
            ...
            transition: dragOffset === 0 ? "transform 1.00s cubic-bezier(0.5, 0.6, 0.5, 1.2)" : "none",
    ...
    return (
        <div
            ...
            style={{
                cursor: hasImages && imageArray.length > 1 ? "grab" : "default",
                height: fit === "height" ? "100%" : undefined,
            }}
        >
            <div className="carousel-track" style={!canRender ? { opacity: 0 } : { ...trackStyle, opacity: 1 }}>
                {!hasImages ? (
                    // ✅ Placeholder (mantiene el DOM/medición y no rompe el carrusel)
                    <div className="carousel-slide-item" style={slideStyle}>
                        <div
                            style={{
                                width: "100%",
                                height: fit === "height" ? "100%" : "auto",
                            }}
                        >
                            {/* Si querés, acá podés poner un skeleton/spinner */}
                            <div className={`relative w-full ${fit === "height" ? "h-full" : ""}`}>
                                {/* Skeleton (fondo) */}
                                <div className="absolute inset-0 animate-pulse bg-slate-800/60" />

                                {/* Detalle suave tipo shimmer (opcional, pero queda lindo) */}
                                <div className="absolute inset-0 bg-gradient-to-br from-transparent via-slate-700/10 to-transparent" />

                                {/* Spinner centrado */}
                                <div className="relative flex w-full items-center justify-center py-10">
                                    {/* Si ya usás lucide-react en el proyecto */}
                                    {/* import { Loader2 } from "lucide-react"; */}
                                    {/* <Loader2 className="h-6 w-6 animate-spin text-slate-300/80" /> */}

                                    {/* Si no querés importar nada, spinner puro CSS (Tailwind) */}
                                    <div className="h-6 w-6 animate-spin rounded-full border-2 border-slate-400/40 border-t-slate-200/80" />
                                </div>
                            </div>
                        ...
                        <div key={img.id} className="carousel-slide-item" style={slideStyle}>
                            <div style={{ width: "100%", height: fit === "height" ? "100%" : "auto" }}>
                                <SitesImageCard
                                    image={img}
                                    isFirst={true}
                                    variant={fit === "height" ? "swiper" : "grid"}
                                />
                            </div>
                        </div>
                    ))
...
export function ImagesSwiperThumbnails({
    ...
    if (!hasImages || imageArray.length <= 1) return null;
